...
public class ApplicationController implements Initializable {
	...
	/**
	 * Metoda wywolywana przy tworzeniu kontrolera (tj. przy ladowaniu definicji
	 * widoku z pliku .fxml). Instaluje procedury obslugi zdarzen klikniecia na
	 * przyciski w glownym menu aplikacji. Jednoczesnie okresla kiedy przyciski
	 * w menu powinny stawac sie aktywne/nieaktywne - decyduje o tym logika
	 * kontrolera aktualnie wybranej zakladki, wiec
	 * {@link pl.edu.mimuw.forum.ui.controller.ApplicationController } tworzy
	 * jedynie zestaw wlasnosci {@link javafx.beans.property.Property} laczace
	 * {@link pl.edu.mimuw.forum.ui.controller.ToolbarController } z
	 * {@link pl.edu.mimuw.forum.ui.controller.MainPaneController } przez
	 * mechanizm wiazan. Warto zapoznac sie z
	 * 
	 * @see <a href=
	 *      "http://docs.oracle.com/javafx/2/binding/jfxpub-binding.htm">
	 *      artykulem o wiazaniach</a> w JavieFX.
	 */
	@Override
	...
	/**
	 * Wyswietla okno dialogowe do wyboru pliku i otwiera wybrany plik w nowej
	 * zakladce.
	 */
	private void open() {
	 ...
	 */
	private void save() {
		when(bindings.getCanSave(),
				() -> getPaneController().ifPresent(controller -> tryExecute("Error saving the file.", () -> {
	...
	/**
	 * Cofa ostatnia wykonana operacje (o ile jest to mozliwe). Za wykonanie
	 * operacji odpowiedzialny jest
	...
	/**
	 * Ponawia wykonanie ostatnia confnieta operacje (o ile jest to mozliwe). Za
	 * ponowienie operacji odpowiedzialny jest
	 * {@link pl.edu.mimuw.forum.ui.controller.MainPaneController }.
	 */
	private void redo() {
	...
	/**
	 * Wyswietla okno dialogowe umozliwiajace dodanie nowego wezla do drzewa. Za
	 * wykonanie operacji odpowiedzialny jest
	 * {@link pl.edu.mimuw.forum.ui.controller.MainPaneController }.
	 */
	private void addNode() {
		when(bindings.getCanAddNode(), () -> {
			...
			dialog.showAndWait().ifPresent(node -> {
				getPaneController().ifPresent(
						controller -> tryExecute("Error adding a new node.", () -> controller.addNode(node)));
			});
		});
	}

	/**
	 * Usuwa aktualnie wybrany wezel drzewa. Za wykonanie operacji
	 * odpowiedzialny jest
	 * {@link pl.edu.mimuw.forum.ui.controller.MainPaneController }.
	 */
	private void deleteNode() {
		when(bindings.getCanDeleteNode(), () -> getPaneController()
				.ifPresent(controller -> tryExecute("Error deleting the node.", controller::deleteNode)));
	}
	...
	private Tab createTab(Node view, MainPaneController controller) {
		...
		Tab tab = new Tab();
		tab.setContent(view);
		// Nazwa zakladki: nazwa pliku i znak * jesli sa na niej niezapisane
		// zmiany
		...
		tab.setOnCloseRequest(evt -> {
			/*
			 * Obsluga zamkniecia zakladki w przypadku, gdy sa na niej
			 * niezapisane zmiany
			 */
