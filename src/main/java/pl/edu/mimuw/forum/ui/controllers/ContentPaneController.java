...
import pl.edu.mimuw.forum.ui.change.CommentChange;
...
public class ContentPaneController extends BasePaneController {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		userProperty().addListener((observable, oldValue, newValue) -> {
			AuthorChange change = new AuthorChange(model, oldValue, newValue);
			change.push(mainPane);
		});

		commentProperty().addListener((observable, oldValue, newValue) -> {
			CommentChange change = new CommentChange(model, oldValue, newValue);
			change.push(mainPane);
		});
	}
